...
if($sure == "yes") {
	...
	$sql = sql_query($query);
	echo("Location successfuly removed, click <a class=altlink href='?'>here</a> to go back.");
	end_frame();
	stdfoot();
	die();
}
$delid = $_GET['delid'];
if($delid > 0) {
	echo("Are you sure you would like to delete this Location?( <strong><a href='?delid=$delid&sure=yes'>Yes!</a></strong> / <strong><a href='?'>No</a></strong> )");
	end_frame();
	stdfoot();
	die();
}
...
if($edited == 1) {
	...
	if(validip_format($start_ip) && validip_format($end_ip))
	{
		if(ip2long($end_ip) > ip2long($start_ip))
		{
			...
			if($sql) 
			{
				stdmsg("Success!","Location has been edited, click <a class=altlink href='?'>here</a> to go back");
				stdfoot();
				die();
			}
		}
...
while ($row = mysql_fetch_array($sql)) {
	...
	"<td class=rowfollow align=center><a href='?editid=$id'>Edit</a></td>".
	"<td class=rowfollow align=center><a href='?delid=$id'>Remove</a></td>" .
	"</tr>");
}
print("</table>");
echo $pagerbottom;
